...
def get_excel_formula_for_risk_level(total_score_cell_id,high_risk_threshold,low_risk_threshold):
    return f"=IF({total_score_cell_id}<{high_risk_threshold},\"HIGH\",IF({total_score_cell_id}<{low_risk_threshold},\"MEDIUM\",\"LOW\"))"


def get_risk_level_and_traffic_light(total_score,high_risk_threshold,low_risk_threshold):
...
def get_human_readable_risk_levels(high_risk_threshold,low_risk_threshold):
